...
from src.collectors.spotify.service import SpotifyAPI
...
_logger.debug("spotify settings: %s", _app_settings)

class SpotifyHandler(BaseHandler, SpotifyAPI):

    def initialize(self):
        super(SpotifyHandler, self).initialize()

    @route(r'/user/spotify/auth/login', HTTP_METHOD.GET, '>=1')
    @coroutine    
    def auth_login(self):        
        ...
        if suid:
            ...
            if auth:
                self.redirect(("/v1/user/%s/spotify/auth" % suid))
                return
        else :
    ...
    @route(r'/user/(.*)/spotify/auth/logout', HTTP_METHOD.GET, '>=1')
    @coroutine    
    ...
    @route(r'/user/(.*)/spotify/auth', HTTP_METHOD.GET, '>=1')
    @coroutine    
    ...
    @route(r'/user/(.*)/spotify/profile', HTTP_METHOD.GET, '>=1')
    @coroutine    
    def get_spotify_profile(self, suid):
        result = yield self.get_profile(suid)
        self.write_result(result)
